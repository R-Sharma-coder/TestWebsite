...
class SimpleHTTPRequestHandler2(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # self.send_header("X-Backend-Server", "12345678")
        # self.send_header("New-Key", "New-Value")
        self.send_header('New-HeaderKey-GET', key.encode("utf-8"))
        # self.send_header('Changed Content-Type', key.encode("utf-8"))
        # send response headers
        self.end_headers()
        # send the body of the response
        self.wfile.write(bytes(file_to_open, 'utf-8'))
    ...
    def write_response(self, content):
        ...
        key = 'application/excel;RP_AD_008_รายงานUserในระบบ_20220225.xls'
        # self.send_header("New-Key", "New-Value")
        self.send_header('New-HeaderKey-POST', key.encode("utf-8"))
        # self.send_header("X-Backend-Server", "12345678")
        self.send_header("Content-type", 'text/html')
        self.end_headers()
        self.wfile.write(content)
...
#     httpd.serve_forever()
# BIND_HOST = "localhost"
# BIND_HOST = "testHeaderServer"
BIND_HOST = "127.0.0.1"
with http.server.HTTPServer((BIND_HOST, 80), SimpleHTTPRequestHandler2) as httpd:
    # print("serving at port", PORT)
    print(f"listening on http://{BIND_HOST}:80")
    httpd.serve_forever()
